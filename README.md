...
The solution is to reset the BIOS to factory settings.  Before executing 
bbswitch for the first time, it is therefore recommended to take note of 
the full key sequence in the BIOS to do a reset.
...
Some machines do not like the cards being disabled at shutdown.  
...
If you have bumblebeed installed, create `/etc/modprobe.d/bbswitch.conf` containing:
